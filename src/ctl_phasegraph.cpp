...
static void
calf_phase_graph_draw_background(GtkWidget *widget, cairo_t *ctx, int sx, int sy, int ox, int oy, float radius, float bevel, float brightness, float shadow, float lights )
{
    int cx = ox + sx / 2;
    int cy = oy + sy / 2;
    
    display_background(widget, ctx, 0, 0, sx, sy, ox, oy, radius, bevel, brightness, shadow, lights);
...
calf_phase_graph_expose (GtkWidget *widget, GdkEventExpose *event)
{
    ...
    if( pg->background == NULL ) {
        ...
        calf_phase_graph_draw_background(widget, ctx_back, sx, sy, ox, oy, radius, bevel, 1, shadow, lights);
