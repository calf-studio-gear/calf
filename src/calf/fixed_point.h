...
public:
    ...
    */
    ...
    inline U lerp_table_lookup_int(U data[(1ULL<<IntBits)+1]) const {
    ...
    template<class U, int UseBits> 
    inline U lerp_table_lookup_int_shift(U data[(1ULL<<IntBits)+1], unsigned int shift) {
        unsigned int pos = (uipart() + shift) & ((1ULL << IntBits) - 1);
        return lerp_by_fract_int<U, UseBits>(data[pos], data[pos+1]);
    }

    template<class U> 
    inline U lerp_table_lookup_float(U data[(1ULL<<IntBits)+1]) const {
        unsigned int pos = uipart();
        return data[pos] + (data[pos+1]-data[pos]) * fpart_as_double();
    }

    template<class U> 
    inline U lerp_table_lookup_float_mask(U data[(1ULL<<IntBits)+1], unsigned int mask) const {
